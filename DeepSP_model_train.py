...
def best_model_SAPpos():    
    ...
    best_model.add(keras.layers.Conv1D(filters=128, kernel_size=5, activation = 'relu', name="Conv1D_1"))
    best_model.add(BatchNormalization())
    best_model.add(keras.layers.Dropout(0.3))

    best_model.add(keras.layers.Conv1D(filters=96, kernel_size=4, activation = 'relu', name="Conv1D_2"))
    best_model.add(BatchNormalization())

    best_model.add(keras.layers.Conv1D(filters=32, kernel_size=5, activation = 'relu', name="Conv1D_3"))
    best_model.add(BatchNormalization())
...
def best_model_SCMpos():    
    ...
    best_model.add(keras.layers.Conv1D(filters=128, kernel_size=4, activation = 'relu', name="Conv1D_1"))
    best_model.add(BatchNormalization())
    best_model.add(keras.layers.Dropout(0.4))

    best_model.add(keras.layers.Conv1D(filters=112, kernel_size=4, activation = 'relu', name="Conv1D_2"))
    best_model.add(BatchNormalization())
    best_model.add(keras.layers.Dropout(0.4))

    best_model.add(keras.layers.Conv1D(filters=144, kernel_size=5, activation = 'relu', name="Conv1D_3"))
    best_model.add(BatchNormalization())
...
def best_model_SCMneg():    
    ...
    best_model.add(keras.layers.Conv1D(filters=64, kernel_size=4, activation = 'relu', name="Conv1D_3"))
    best_model.add(BatchNormalization())
...
for file, model, l_rate in zip(filenames,models,l_rates):
    ...
    # Create callback
    filepath = 'Conv1D_regression_'+prop+'.h5'
    ...
    history = best_model.fit(x=X_train, y=y_train, shuffle=True, validation_data=(X_val, y_val), epochs=50, callbacks=callbacks, batch_size=64, verbose=2)
    ...
    reg = ['CDRH1', 'CDRH2', 'CDRH3', 'CDRL1', 'CDRL2', 'CDRL3', 'CDR', 'Hv', 'Lv', 'Fv']
    ...
    for r, i, j, k, l in zip(reg, mean_score_list, baseline_mae_list, mae_list, corr_list):
    ...
    result_df.to_csv("hyp_metric_" + prop + ".csv", index=False)
...
concatenated_df.to_csv("Final_model_metric.csv", index=False)
