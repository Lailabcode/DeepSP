...
# Import dataset
dataset = pd.read_csv(
    "../data/DeepSP_input.csv"
)  # replace with your csv file, see format in DeepSP_input.csv file
...
# sequence preprocessing: source - https://github.com/Lailabcode/DeepSCM/blob/main/deepscm-master/seq_preprocessing.py
...
df = pd.concat(
    [
        ...
        pd.DataFrame(sap_pos),
        pd.DataFrame(scm_neg),
        pd.DataFrame(scm_pos),
    ],
    ignore_index=True,
    axis=1,
)
df.columns = features
df.to_csv("../data/DeepSP_descriptors.csv", index=False)
